...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("xraycoin"))
        return false;
...
{
    // Convert xraycoin:// to xraycoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("xraycoin://"))
    {
        uri.replace(0, 12, "xraycoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "XrayCoin.lnk";
}
...
{
    return GetAutostartDir() / "xraycoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("XrayCoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  xraycoin-qt [" + tr("command-line options") + "]                     " + "\n";
